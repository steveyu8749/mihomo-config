...
Mihomo V5.1 面向手机与电脑本机运行 Mihomo 的场景。V5.1 的项目层变化集中在修正与补充 Shadowrocket；本文件描述的 Mihomo 流量语义保持不变。设计优先级依次是：
...
模板使用公共 MRS 与从自维护文本生成的 MRS Rule Provider，不依赖客户端内置 GeoData；节点完全手工选择，不维护地区筛选和自动策略组。
...
| 规则数据 | domain / ipcidr text 作为源，Actions 生成 MRS，不配置 GeoData |
| Adobe | 公共模板默认完全关闭 |
...
`cache-algorithm` 支持默认 LRU 和可选 ARC。V5.1 保持参数缺省，继续使用 LRU。没有设备侧缓存命中数据时，不假定 ARC 一定更优，也不为未经验证的收益增加配置分支。
...
V5.1 的自维护 `direct_domain` 位于全部专用业务域名之后、ProxyLite 之前。它是硬直连补充集，不是日常直连全集：普通中国域名和中国 IP 由 `cn_domain` / `cn_ip` 处理。当前 36 条完整覆盖原有的 ScienceDirect、Elsevier、Clarivate / Web of Science 资源，并补充高频科研入口，用于校园或机构出口 IP 认证；不复制 PT、下载进程、国内大厂域名或完整 Scholar 大类。
...
V4.13 对实际 ProxyLite 做了覆盖审计：删除已由专用/MRS 集合接管的条目和旧国外 DNS 清单；把仍会误命中 `zhubajie` 的 `DOMAIN-KEYWORD,bajie` 收窄为两个明确后缀。V4.14 将留下的 7 条域名与 2 条精确 IP 拆分为 `ProxyLite.list` 和 `ProxyIP.list`；后者使用 `ipcidr` 且路由带 `no-resolve`，不为匹配精确地址额外解析域名。
...
自维护 Direct  → domain text 源 → domain MRS Rule Provider
ProxyLite      → domain text 源 → domain MRS Rule Provider
ProxyIP        → ipcidr text 源 → ipcidr MRS Rule Provider
Fake-IP 兼容   → domain text 源 → domain MRS Rule Provider
```
...
MRS 只支持 `domain` 与 `ipcidr` behavior。因此启用中的自维护文本都使用这两种格式，CI 会将四份文本源转成临时 MRS 做核心验证。Adobe 是唯一保留的 classical 例外，且默认不启用。
...
V5.1 不配置 `GEOSITE`、`GEOIP`、`geodata-mode`、`geo-auto-update` 或 `geox-url`。
...
- 自维护 Direct / Fake-IP / ProxyLite / ProxyIP 文本规则的 behavior、语法、去重、必要条目与 Provider URL；
